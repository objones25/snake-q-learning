...
uv run python main.py train --no-shield                                # disable the flood-fill safety shield during training
uv run python main.py train --resume-from other.json                   # warm-start from an existing q_table instead of training from scratch
...
uv run python main.py play --no-shield     # disable the flood-fill safety shield while playing
...
A bottom-layer module, `config.py`, sits alongside `snake_types.py` — frozen dataclasses (`AgentConfig`, `RenderConfig`, `TrainConfig`, `PlayConfig`) that any layer imports for its own parameters, replacing what used to be scalar kwargs duplicated across `main.py`/`watch.py`/`train.py`/`play.py`/`q_agent.py`/`renderer.py`. `QLearningAgent` takes an `AgentConfig`, `PygameRenderer` takes a `RenderConfig` — but `train()`/`play()` themselves no longer take a `TrainConfig`/`PlayConfig` directly; their signature is `(env, agent, n_episodes, use_shield: bool = True)`. `main.py`/`watch.py`/`api.py` each own building the `env`/`agent` from a `TrainConfig`/`PlayConfig` (`TrainConfig` itself carries an `AgentConfig`) before calling `train()`/`play()`. See `docs/superpowers/specs/2026-07-29-centralized-config-design.md`.

Also alongside the bottom layers, `safety.py` is an environment-agnostic BFS flood-fill safety shield: `safe_action_mask(body, direction, food, grid_size) -> tuple[bool, bool, bool]` returns a per-`Action` mask marking which of the snake's 3 candidate actions are safe — no immediate collision, and don't trap the snake in a pocket too small to fit its own body afterward (a BFS flood-fill from the resulting head, short-circuited once enough reachable cells are confirmed). Unlike `renderer.py`/`plotting.py`/`api.py`, this isn't an optional side branch: `train()`/`play()` wire the mask into `QLearningAgent.choose_action` (restricting exploration/greedy-pick) and, for `train()`, `QLearningAgent.update` (restricting the Bellman-bootstrap max to the *next* state's mask) — on by default via each function's `use_shield: bool = True` parameter. `SnakeEnv.safe_action_mask()` packages up `self.snake`/`self.food`/`self.grid_size` into the call to `safety.safe_action_mask`, mirroring `render_state()`'s pattern, so `train.py`/`play.py` call `env.safe_action_mask()` rather than reaching into `env.snake` directly; `safety.py` itself imports nothing from `SnakeEnv` and stays environment-agnostic.
...
- **`QLearningAgent`** is deliberately decoupled from everything above — it only needs `SnakeState.N_STATES` and an `AgentConfig` (`config.py`) at construction and thereafter only sees integer state indices and `Action` values. Q-table is a plain `list[list[float]]`, persisted as unversioned JSON (`save`/`load`, path-typed via `pathlib.Path`) with no shape validation against `n_states`/`n_actions`. `choose_action`/`update` both take an optional `mask`/`next_mask` (a per-`Action` boolean tuple, see `safety.safe_action_mask`) that restricts exploration/greedy-pick and the Bellman-bootstrap max respectively to shield-approved actions — falling back to the full, unrestricted set if every action is masked out (fully boxed in, nothing safe left to restrict to).
- **`play.py`** mirrors `train.py`'s episode loop but takes an already-loaded `agent` and never calls `agent.update` — pure greedy inference, no learning; callers (`main.py`, `watch.py`, `api.py`) own construction, loading, and the `epsilon = 0.0` reset before calling it. The fail-fast `FileNotFoundError` check for a missing Q-table path (rather than surfacing `json.load`'s raw traceback) lives in those callers, not in `play.py` itself.

`train.py`, `play.py`, and `main.py` never touch `Snake` or grid geometry directly — only `SnakeEnv` and `SnakeState` (`SnakeEnv.safe_action_mask()` covers this for the shield too — see above).
...
**Collision/tail-exclusion logic is duplicated, not shared**, between `SnakeEnv.step` (excludes tail from the obstacle set _unless_ food was just consumed this move, since then the tail doesn't vacate), `SnakeState.from_world`'s danger ray-casting (unconditionally excludes the tail), and now `safety.py`'s `_resolve_action` (the shield's one-step lookahead, same tail-vacate rule). They happen to agree in practice but nothing enforces it — check all three when touching collision or danger-sensing logic. `tests/test_train.py::TestShieldSafetyGuarantee` is a canary specifically for `safety.py` drifting from `SnakeEnv.step`'s real rules: it asserts every real death across a run only happens when the shield's mask was all-`False`, which would start failing if the two disagreed (i.e. the shield approved a move that `SnakeEnv.step` actually kills the snake for).
